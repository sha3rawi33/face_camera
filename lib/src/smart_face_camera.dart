...
class SmartFaceCamera extends StatefulWidget {
  ...
  const SmartFaceCamera({required this.controller,
...
class _SmartFaceCameraState extends State<SmartFaceCamera>
  ...
  Color? get iconColor =>
      widget.controller.enableControls ? null : Theme
          .of(context)
          .disabledColor;

  /// Display the control buttons to take pictures.
  Widget _captureControlWidget(FaceCameraState value) {
    return widget.captureControlBuilder?.call(context, value.detectedFace) ?? IconButton(
      icon: CircleAvatar(
          radius: 35,
          foregroundColor:
